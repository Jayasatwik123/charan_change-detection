...
class ChangeDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if random.random() > 0.5:
            image_A = image_A.transpose(Image.FLIP_TOP_BOTTOM)
            image_B = image_B.transpose(Image.FLIP_TOP_BOTTOM)
        ...
        if random.random() > 0.7:
            enhancer = ImageEnhance.Brightness(image_A)
            image_A = enhancer.enhance(random.uniform(0.7, 1.3))
            enhancer = ImageEnhance.Brightness(image_B)
            image_B = enhancer.enhance(random.uniform(0.7, 1.3))
            enhancer = ImageEnhance.Contrast(image_A)
            image_A = enhancer.enhance(random.uniform(0.7, 1.3))
            enhancer = ImageEnhance.Contrast(image_B)
            image_B = enhancer.enhance(random.uniform(0.7, 1.3))
        # Random cutout
        if random.random() > 0.7:
            def cutout(img, size=32):
                ...
                img.paste(0, (x0, y0, x0 + size, y0 + size))
                return img
            image_A = cutout(image_A)
            image_B = cutout(image_B)
            gt = cutout(gt)
...
class Test_ChangeDataset(data.Dataset):
    def __init__(self, root, trainsize):
        ...
        self.images_A = [os.path.join(image_root_A, f) for f in os.listdir(image_root_A) if f.endswith('.jpg') or f.endswith('.png')]
        self.images_B = [os.path.join(image_root_B, f) for f in os.listdir(image_root_B) if f.endswith('.jpg') or f.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        image_B = self.img_transform(image_B)
        gt = self.gt_transform(gt)
        # Ensure file_name is just the base name, without path or extension
        base_name_with_ext = os.path.basename(self.images_A[index])
